...
<div class="container-fluid text-center">
  ...
  <form method="POST" action="top-product-rm.php">
   ...
   <?php } ?>
    ...
    <button type="submit" class="button_unit" name="product_RM"><b>In RM</b></button>
    </div>
  </form>
  <br>
    </div>
...
<?php
  ...
  $handle = $link->prepare("SELECT p.name, p.category, SUM(o.Qty*o.price) AS totalRM FROM product p JOIN order_details o ON p.ProductID= o.ProductID WHERE year(o.OrderedTime)='$product_year' and MonthName(o.OrderedTime)='$product_month' and p.category='$category' GROUP BY p.name ORDER BY totalRM desc limit 3");
 ...
 foreach($result as $row){
    array_push($dataPoints, array("label"=> $row->name, "y"=> $row->totalRM));
